    ...
    return (
      <main className="relative z-10 mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="rounded-3xl border border-dashed border-[#d0bf9f] bg-[#fbf6ea] py-16 text-center">
          <h2 className="text-2xl font-black text-[#1f3148]">Ministri nuk u gjet</h2>
          <Link
            href="/qeveria"
    ...
    <main className="relative z-10 mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 md:py-14">
        ...
        <div className="relative">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-4">
            <div className="rounded-2xl border border-[#d6c9af] bg-[#faf6ed] p-4">
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#876e46]">Score</p>
              <p className={`mt-2 text-3xl font-black ${scoreTone(ministry.score)}`}>{ministry.score}/100</p>
            </div>
            <div className="rounded-2xl border border-[#d6c9af] bg-[#faf6ed] p-4">
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#876e46]">Mes. progres</p>
              <p className="mt-2 text-3xl font-black text-[#163755]">{ministry.avgProgress}%</p>
            </div>
            <div className="rounded-2xl border border-[#d6c9af] bg-[#faf6ed] p-4">
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#876e46]">Kartat aktive</p>
              <p className="mt-2 text-3xl font-black text-[#163755]">{ministry.activeCount}</p>
            </div>
            <div className="rounded-2xl border border-[#d6c9af] bg-[#faf6ed] p-4">
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#876e46]">Kartat e lidhura</p>
      ...
      <section className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr]">
        <div className="rounded-[2rem] border border-[#d7cab2] bg-[#f8f3e8] p-6">
          <h2 className="text-2xl font-black text-[#163654]">Kartat e lidhura me këtë ministri</h2>
          <p className="mt-2 text-sm font-semibold text-[#5e6f86]">
            Lista është baza e score-it. Çdo kartë ka pikë sipas progresit, statusit dhe freskisë së përditësimeve.
          </p>
            ...
            {ministry.promiseScores.map((entry) => (
              <article key={entry.promise.id} className="rounded-2xl border border-[#d8cab2] bg-[#fcf8ef] p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-[0.15em] text-[#7a6032]">{entry.promise.category}</p>
                ...
                <div className="mt-3 flex items-end justify-between">
                  <p className="text-sm font-bold text-[#5f7088]">Progresi: {entry.promise.progress}%</p>
                  <p className={`text-lg font-black ${scoreTone(entry.score)}`}>Pikë: {entry.score}</p>
                </div>
                ...
                <div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-xs font-semibold text-[#627289]">
                  <span>Përditësimi i fundit: {formatDate(entry.lastUpdateDate)}</span>
        ...
        <div className="space-y-8">
          <section className="rounded-[2rem] border border-[#d7cab2] bg-[#f8f3e8] p-6">
            <h3 className="text-xl font-black text-[#153451]">Përbërja e statusit</h3>
          ...
          <section className="rounded-[2rem] border border-[#d7cab2] bg-[#f8f3e8] p-6">
            <h3 className="text-xl font-black text-[#153451]">Fushat kryesore</h3>
            ...
            <h4 className="mt-6 text-sm font-black uppercase tracking-[0.14em] text-[#7a6032]">Mapping i përdorur</h4>
            <p className="mt-2 text-sm font-semibold text-[#60718a]">Karta të lidhura: {ministry.config.promiseIds.join(', ')}</p>
          </section>

          <section className="rounded-[2rem] border border-[#d7cab2] bg-[#f8f3e8] p-6">
            <h3 className="text-xl font-black text-[#153451]">Përditësime të fundit</h3>
            {ministry.recentUpdates.length > 0 ? (
              <div className="mt-4 space-y-3">
                {ministry.recentUpdates.slice(0, 6).map((update, index) => (
                  <article key={`${update.promiseId}-${update.date}-${index}`} className="rounded-xl border border-[#d8cab2] bg-[#fcf8ef] p-3">
                    <p className="text-[10px] font-black uppercase tracking-[0.15em] text-[#7a6032]">
                      {formatDate(update.date)} • Kartela #{update.promiseId}
                    </p>
              ...
              <p className="mt-4 text-sm font-semibold text-[#60718a]">Nuk ka përditësime për kartat e lidhura.</p>
            )}
          </section>
        </div>
      </section>
