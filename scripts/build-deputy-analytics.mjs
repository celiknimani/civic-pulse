...
const deputiesPath = path.resolve(readArg(argv, '--deputies', countryPath(country, 'deputies.csv')));
const transcriptsDir = path.resolve(readArg(argv, '--transcripts', countryPath(country, 'transcripts')));
const outputPath = path.resolve(readArg(argv, '--out', publicDataPath('deputies-analytics.json')));
const categoriesPath = path.resolve(readArg(argv, '--categories', countryPath(country, 'categories.ts')));

const loadDeputyTopicTaxonomy = async () => {
  const jsonCandidate = categoriesPath.replace(/\.ts$/i, '.json');
  try {
    const raw = await fs.readFile(jsonCandidate, 'utf8');
    const parsed = JSON.parse(raw);
    if (Array.isArray(parsed)) return parsed;
    if (Array.isArray(parsed.taxonomy)) return parsed.taxonomy;
    if (Array.isArray(parsed.categories)) return parsed.categories;
  } catch {
    // fall through to TS module
  }
  const moduleUrl = pathToFileURL(categoriesPath).href;
  const moduleExports = await import(moduleUrl);
  if (!Array.isArray(moduleExports.DEPUTY_TOPIC_TAXONOMY)) {
    throw new Error(`${categoriesPath} must export DEPUTY_TOPIC_TAXONOMY array.`);
  }
  return moduleExports.DEPUTY_TOPIC_TAXONOMY;
};

const DEPUTY_TOPIC_TAXONOMY = await loadDeputyTopicTaxonomy();
...
const writeDatasetIfChanged = async (absolutePath, dataset) => {
  try {
    const existing = JSON.parse(await fs.readFile(absolutePath, 'utf8'));
    if (JSON.stringify(stripGeneratedAt(existing)) === JSON.stringify(stripGeneratedAt(dataset))) {
      return false;
    }
  } catch (_error) {
  ...
  await fs.writeFile(absolutePath, JSON.stringify(dataset, null, 2), 'utf8');
  return true;
};
...
const build = async () => {
  ...
  const wroteOutput = await writeDatasetIfChanged(outputPath, dataset);

  const activeDeputies = dataset.deputies.filter((deputy) => deputy.activity.speechCount > 0).length;
  console.log(`[${country}] ${wroteOutput ? 'Saved' : 'Unchanged'} ${dataset.deputies.length} deputies at ${outputPath}`);
