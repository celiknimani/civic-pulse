...
const inputPath = path.resolve(readArg(argv, '--in', countryPath(country, 'sources', 'promises.md')));
const outputPath = path.resolve(readArg(argv, '--out', publicDataPath('promise-sources.json')));
...
const parsePromiseIds = (value) => {
  ...
  if (parsed.some((entry) => !/^[\w-]+$/.test(entry))) {
...
const validateSourceType = (value) => {
  const allowed = new Set(['government_website', 'minister_social', 'trusted_news', 'parliament_transcript', 'official_gazette']);
...
const validateFetchMode = (value) => {
  const allowed = new Set(['html', 'rss', 'search', 'manual', 'pdf']);
  if (!allowed.has(value)) {
...
const build = async () => {
  ...
  console.log(`[${country}] ${wroteOutput ? 'Saved' : 'Unchanged'} ${sources.length} promise sources at ${outputPath}`);
};
